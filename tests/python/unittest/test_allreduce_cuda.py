...
@T.prim_func
def lt(a: T.handle, b: T.handle, d1: T.int32, d2: T.int32, d3: T.int32) -> None:
    A = T.match_buffer(a, [1, d1, d2, d3])
    B = T.match_buffer(b, [1, d1, d2, d3])

    for i, j, k, l in T.grid(1, d1, d2, d3):
        with T.block("lt"):
            # target of Assign's dtype is tuple
            vi, vj, vk, vl = T.axis.remap("SSSS", [i, j, k, l])
            -A[vi, vj, vk, vl]
            # B[vi, vj, vk, vl] =  -A[vi, vj, vk, vl] 
...
def test_lt():
    _, _, _d1, _d2, _d3 = lt.params
    d1 = 32
    d2 = 32
    d3 = 32
    mod = lt.specialize({_d1: d1, _d2: d2, _d3: d3})
    sch = tvm.tir.Schedule(mod)
    blk = sch.get_block("lt")
    i, j, k, l = sch.get_loops(blk)
    sch.bind(i, "blockIdx.x")
    sch.bind(j, "threadIdx.z")
    sch.bind(k, "threadIdx.y")
    sch.bind(l, "threadIdx.x")
    tir_mod = sch.mod["main"]
    # print(tir_mod.__repr__())
    print(tir_mod)
    f = tvm.build(sch.mod["main"], target="cuda")
    ...
    # a_np = np.random.rand(1, d1, d2, d3).astype("float32")
    # a = tvm.nd.array(a_np, tvm.cuda(0))
    # b = tvm.nd.array(np.zeros_like(a_np), tvm.cuda(0))
